...
def get_timeframes_frequency(court_id, request):
    ...
    for timeframe in court_timeframes:
        ...
        for interval in intervals:
            ...
            if check_if_date_with_timedelta_is_between_two_dates(interval, timedelta(minutes=30), timeframe.start, timeframe.end):
...
def sort_list(data, key, reverse=False):
    logging.debug(f"Sorting list by key = '{key}', reverse = '{reverse}'")
...
def get_courts_list(request, serializer):
    ...
    logging.debug(f"Getting courts list, request query parameters = {request.query_params}")
    latitude = parse_parameter_from_query_parameters(request, 'lat', DataType.FLOAT)
    longitude = parse_parameter_from_query_parameters(request, 'lon', DataType.FLOAT)
    logging.debug(f"Getting courts with set distance from - lat = {latitude}, lon = {longitude}")
    ...
    logging.debug(f"Order query params: 'order_by' = {order_by}, 'reverse' = {reverse_order}")
    try:
        ...
        logging.debug(f"Not possible to sort because = {str(e)}")
...
def check_if_date_with_timedelta_is_between_two_dates(date, delta, start_date, end_date):
    data = utc.localize(date)
    logging.debug(f"Checking if interval - {data} - {data + delta} is between {start_date} and {end_date}")
    if data >= start_date and data + delta <= end_date:
        return True
    else:
        return False
...
def calculate_distance_between_two_coordinates(lat_1, lon_1, lat_2, lon_2):
    ...
    logging.debug(f"Calculating distance between ({lat_1}, {lon_1}) and ({lat_2}, {lon_2})")
